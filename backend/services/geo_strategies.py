...
STRATEGY_PROMPTS: Dict[str, str] = {
    ...
    "topic_cluster_alignment": (
        "请检查以下内容是否符合 Topic Cluster 最佳实践，并进行优化。\n"
        "要求：\n"
        "1. 检查内容是否围绕一个核心支柱主题展开\n"
        "2. 识别可以拆分为集群子页面的话题\n"
        "3. 添加自然的内部链接锚文本（用 [锚文本](链接目标描述) 标注）\n"
        "4. 确保每个子话题有明确的搜索意图定位\n"
        "5. 优化标题层级以匹配 Topic Cluster 结构\n\n"
        "输出格式：\n"
        "## Topic Cluster 分析\n"
        "| 层级 | 主题 | 搜索意图 | 内链建议 |\n"
        "|------|------|---------|----------|\n"
...
def _estimate_improvement(strategy_name: str) -> str:
    estimates = {
        ...
        "topic_cluster_alignment": "内容结构化和内链质量提升约 20-30%",
    }
